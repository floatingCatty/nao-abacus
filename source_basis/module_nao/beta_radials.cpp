...
// void BetaRadials::build(const Numerical_Nonlocal& nl, const int itype, const ModuleBase::Logger* const ptr_logger)
// {
//     cleanup();
...
void BetaRadials::build(const std::string& file, const bool lspinorb, const int itype, const int p, const int pm, const ModuleBase::Logger* ptr_logger, MPI_Comm comm)
{
    /*
    ...
    if (!is_open)
    {
        throw std::runtime_error("BetaRadials::read: Couldn't open pseudopotential file: " + file);
    }
    ...
    if (ptr_logger)
    {
        ...
        ptr_logger->info() << " |                                                                   |" << std::endl;
        ptr_logger->info() << " |       SETUP BETA PROJECTORS FOR TWO-CENTER INTEGRALS              |" << std::endl;
        ptr_logger->info() << " |                                                                   |" << std::endl;
        ptr_logger->info() << " | Projector information includes the cutoff radius, angular         |" << std::endl;
        ptr_logger->info() << " | momentum, zeta number and numerical values on a radial grid.      |" << std::endl;
        ptr_logger->info() << " |                                                                   |" << std::endl;
        ptr_logger->info() << " <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" << std::endl;
        ptr_logger->info() << "\n\n\n\n";
    }
...
#ifdef __MPI
    Parallel_Common::bcast_int(upf_version, comm);
#endif

    switch (upf_version)
    {
    case 100:
        read_beta_upf100(ifs, ptr_logger, comm);
        break;
    case 201:
        read_beta_upf201(ifs, lspinorb, ptr_logger, comm);
        break;
...
void BetaRadials::read_beta_upf100(std::ifstream& ifs, const ModuleBase::Logger* ptr_logger, MPI_Comm comm)
{
    /*
...
#ifdef __MPI
    Parallel_Common::bcast_bool(is_good, comm);
    Parallel_Common::bcast_string(symbol_, comm);
    Parallel_Common::bcast_int(lmax_, comm);
    Parallel_Common::bcast_int(nchi_, comm);
    Parallel_Common::bcast_int(ngrid_max, comm);
#endif

    if (!is_good)
    {
        throw std::runtime_error("BetaRadials::read_beta_upf100: PP_HEADER error");
    }
...
#ifdef __MPI
    Parallel_Common::bcast_double(rgrid, ngrid_max, comm);
#endif
...
#ifdef __MPI
        Parallel_Common::bcast_int(l, comm);
        Parallel_Common::bcast_int(ngrid, comm);
        Parallel_Common::bcast_int(izeta, comm);
        Parallel_Common::bcast_double(rbeta, ngrid, comm);
#endif
        lmin_ = std::min(lmin_, l);
...
*
*======================================================================*/
void BetaRadials::read_beta_upf201(std::ifstream& ifs, bool lspinorb, const ModuleBase::Logger* ptr_logger, MPI_Comm comm)
{
   /*
   ...
   if (!is_good)
   {
       throw std::runtime_error("BetaRadials::read_beta_upf201: PP_HEADER error");
   }
   ...
   if (!has_so && lspinorb)
   {
       throw std::runtime_error("BetaRadials::read_beta_upf201: lspinorb is set to true but the pseudopotential file does not contain spin-orbit information");
   }
   ...
   if (!is_good)
   {
       throw std::runtime_error("BetaRadials::read_beta_upf201: PP_BETA error");
   }
    ...
    *===========================================================*/
#ifdef __MPI
   Parallel_Common::bcast_int(nbeta_final, comm);
#endif
...
#ifdef __MPI
   Parallel_Common::bcast_int(l_final, nbeta_final, comm);
   Parallel_Common::bcast_int(ngrid_final, nbeta_final, comm);
   Parallel_Common::bcast_double(rbeta_final, nbeta_final * ngrid_max, comm);
#endif
   ...
   for (int i = 0; i != nchi_; ++i)
   {
       ...
       lmin_ = std::min(lmin_, l_final[i]);
       chi_[i].build(l_final[i], true, ngrid_final[i], rgrid, &rbeta_final[i * ngrid_max], 1, izeta, symbol_,
       itype_);

       if (l_final[i] > lmax_) {
           throw std::runtime_error("BetaRadials::read_beta_upf201: l > lmax");
       }
       nzeta_[l_final[i]]++;
       norb_[l_final[i]] = nzeta_[l_final[i]] * (2 * l_final[i] + 1);
       nphi_ += (2 * l_final[i] + 1);
   }
