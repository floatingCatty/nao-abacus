...
void Pseudopot_upf::complete_default(Atom_pseudo& pp, double rcut)
{
    // ModuleBase::TITLE("Pseudopot_upf", "complete_default");

    // call subroutines
    this->complete_default_h(pp);
	this->complete_default_atom(pp, rcut);
...
void Pseudopot_upf::complete_default_atom(Atom_pseudo& pp, double rcut)
{
	...
	if(pp.rcut <= 0.0)
	{
		throw std::runtime_error("Pseudopot_upf::complete_default_atom, PAO rcut<=0.0.");
