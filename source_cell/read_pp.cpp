...
int Pseudopot_upf::init_pseudo_reader(const std::string &fn, std::string &type, Atom_pseudo& pp)
{
	...
	else if (type == "vwr")
	{
        ...
        std::cerr << "VWR format not supported in this standalone version." << std::endl;
        return 1;
	}
	else if (type == "upf201")
	{
	...
	else if (type == "blps")
	{
        ...
        std::cerr << "BLPS format not supported in this standalone version." << std::endl;
        return 1;
	}
    else
    {
...
int Pseudopot_upf::average_p(const double& lambda, const bool lspinorb, Atom_pseudo& pp)
{
    int error = 0;
    double lambda_ = lambda;
    if(!lspinorb) { lambda_ = 0.0; }
    ...
    if (!pp.has_so || (lspinorb && std::abs(lambda_ - 1.0) < 1.0e-8))
    {
        return error;
    }
