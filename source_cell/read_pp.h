...
public:
    ...
    int average_p(const double& lambda, const bool lspinorb, Atom_pseudo& pp); // zhengdy add 2020-10-20
    ...
    void complete_default(Atom_pseudo& pp, double rcut);
  ...
  private:
    ...
    void complete_default_atom(Atom_pseudo& pp, double rcut);
    void complete_default_vl(Atom_pseudo& pp);
};

inline bool SCAN_BEGIN(std::ifstream &ifs, 
                ...
                const bool ifwarn = true)
{
    ...
    if (!find && ifwarn)
    {
        throw std::runtime_error("In SCAN_BEGIN, can't find: " + TargetName + " block.");
    }
    return find;
}

inline void SCAN_END(std::ifstream &ifs, const std::string &TargetName, const bool ifwarn = true)
{
    std::string SearchName;
    ifs >> SearchName;
    if (SearchName != TargetName && ifwarn)
    {
        throw std::runtime_error("In SCAN_END, can't find: " + TargetName + " block.");
    }
    return;
}
