...
{
    if (spin != 0 && spin != 1)
        throw std::runtime_error("spin direction unknown");
    if (m < -l - 1 || m > l)
        throw std::runtime_error("m not allowed");
    ...
    {
        throw std::runtime_error("j and l not compatible");
    }
...
int Soc::sph_ind(const int l, const double j, const int m, const int spin)
{
    ...
    if (spin != 0 && spin != 1)
    {
        throw std::runtime_error("spin must be 0 1");
    }
    if (m < -l - 1 || m > l)
    {
        throw std::runtime_error("m not allowed");
    ...
    else
    {
        ...
        throw std::runtime_error("l and j not suitable");
    }
                    ...
                    const int &ip2)
{
    ...
    for (int m = -l1 - 1; m < l1 + 1; m++)
    {
        ...
        coeff += rotylm(m1, mi) * spinor(l1, j1, m, is1) * std::conj(rotylm(m2, mj)) * spinor(l2, j2, m, is2);
    }
