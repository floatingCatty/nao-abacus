...
int Pseudopot_upf::read_pseudo_upf201(std::ifstream &ifs, Atom_pseudo& pp)
{
    ...
    if (pp.nlcc)
    {
        if (SCAN_BEGIN(ifs, "<PP_NLCC", true, false))
        {
    ...
    {
        if (SCAN_BEGIN(ifs, "<PP_LOCAL", true, false))
        {
            ifs.ignore(150, '>'); // skip type, size, columns and so on.
        }
        else
    ...
    // std::cout << "DEBUG: nchi=" << pp.nchi << " nbeta=" << pp.nbeta << " pp_type='" << pp.pp_type << "'" << std::endl;
    ...
    //-          PP_RHOATOM                -
    //--------------------------------------
    if (SCAN_BEGIN(ifs, "<PP_RHOATOM", true, false))
    {
        ifs.ignore(150, '>');
    }
    else
...
void Pseudopot_upf::getnameval(std::ifstream& ifs, int& n, std::string* name, std::string* val)
{
    ...
    for (int i = 0; i < n; ++i)
    {
		...
		if (!findmark)
		{
            throw std::runtime_error("Pseudopot_upf::getnameval, The values are not in \' or \". Please improve the program in read_pp_upf201.cpp");
...
{
	if (!SCAN_BEGIN(ifs, "<PP_HEADER"))
	{
        throw std::runtime_error("Pseudopot_upf::read_pseudo_upf201_header, Found no PP_HEADER");
    ...
    for (int ip = 0; ip < nparameter; ++ip)
    {
        ...
        else if (name[ip] == "pseudo_type")
        {
            ...
            if (pp.pp_type == "SL")
            {
                throw std::runtime_error("Pseudopot_upf::read_pseudo_upf201_header, SEMI-LOCAL PSEUDOPOTENTIAL IS NOT SUPPORTED");
        ...
        else if (name[ip] == "is_paw")
        {
            if (val[ip] == "T" || val[ip] == "TRUE" || val[ip] == "True" || val[ip] == "true")
            {
                throw std::runtime_error("Pseudopot_upf::read_pseudo_upf201_header, PAW POTENTIAL IS NOT SUPPORTED");
        ...
        else
        {
            ...
            throw std::runtime_error("PP_HEADRER reading, " + warningstr);
...
void Pseudopot_upf::read_pseudo_upf201_mesh(std::ifstream& ifs, Atom_pseudo& pp)
{
    ...
    if (SCAN_BEGIN(ifs, "<PP_MESH", true, false))
    {
        this->getnameval(ifs, nparameter, name, val);
        ...
        for (int ip = 0; ip < nparameter; ++ip)
        {
            ...
            else
            {
                std::string warningstr
                    = name[ip] + " is not read in. Please add this parameter in read_pp_upf201.cpp if needed.";
                throw std::runtime_error("PP_MESH reading, "+warningstr);
    ...
    if (SCAN_BEGIN(ifs, "<PP_R", true, false))
    {
        ifs.ignore(150, '>');
    }
    else
    ...
    if (SCAN_BEGIN(ifs, "<PP_RAB", true, false))
    {
        ifs.ignore(150, '>');
    }
    else
...
void Pseudopot_upf::read_pseudo_upf201_nonlocal(std::ifstream& ifs, Atom_pseudo& pp)
{
    ...
    for (int ib = 0; ib < pp.nbeta; ib++)
    {
        ...
        for (int ip = 0; ip < nparameter; ++ip)
        {
            ...
            else
            {
                std::string warningstr
                    = name[ip] + " is not read in. Please add this parameter in read_pp_upf201.cpp if needed.";
                throw std::runtime_error("PP_BETA reading, "+warningstr);
    ...
    if (SCAN_BEGIN(ifs, "<PP_DIJ", true, false))
    {
        ifs.ignore(150, '>');
    }
    else
    ...
    if (pp.tvanp && SCAN_BEGIN(ifs, "<PP_AUGMENTATION"))
    {
        this->getnameval(ifs, nparameter, name, val);
        // default value
        pp.nqlc = 2 * pp.lmax + 1;
        for (int ip = 0; ip < nparameter; ++ip)
        {
            ...
            else
            {
                std::string warningstr
                    = name[ip] + " is not read in. Please add this parameter in read_pp_upf201.cpp if needed.";
                throw std::runtime_error("PP_AUGMENTATION reading, "+warningstr);
                // ModuleBase::WARNING("PP_AUGMENTATION reading", warningstr);
            }
        }

        // PP_Q
        if (SCAN_BEGIN(ifs, "<PP_Q", true, false))
        {
            ifs.ignore(150, '>');
        }
        else
    ...
    for (int iw = 0; iw < pp.nchi; iw++)
    {
        ...
        for (int ip = 0; ip < nparameter; ++ip)
        {
            ...
            else
            {
                std::string warningstr
                    = name[ip] + " is not read in. Please add this parameter in read_pp_upf201.cpp if needed.";
                throw std::runtime_error("PP_CHI reading, "+warningstr);
    ...
    for (int nw = 0; nw < pp.nchi; nw++)
    {
        ...
        for (int ip = 0; ip < nparameter; ++ip)
        {
            ...
            else
            {
                std::string warningstr
                    = name[ip] + " is not read in. Please add this parameter in read_pp_upf201.cpp if needed.";
                throw std::runtime_error("PP_RELWFC reading, "+warningstr);
    ...
    for (int nb = 0; nb < pp.nbeta; nb++)
    {
        ...
        for (int ip = 0; ip < nparameter; ++ip)
        {
            ...
            else
            {
                std::string warningstr
                    = name[ip] + " is not read in. Please add this parameter in read_pp_upf201.cpp if needed.";
                throw std::runtime_error("PP_RELBETA reading, "+warningstr);
