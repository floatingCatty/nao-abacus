...
{
    READ_VALUE(ifs, pp.nv);  // Version number
    READ_VALUE(ifs, pp.psd); // Element label

    // Type of pseudo : NC or US
    READ_VALUE(ifs, pp.pp_type);
    if (pp.pp_type == "US")
    {
        pp.tvanp = true;
    ...
    else
    {
        ...
        throw std::runtime_error("Pseudopot_upf::read_pseudo_header, unknown pseudo type");
    ...
    READ_VALUE(ifs, pp.xc_func);

    READ_VALUE(ifs, pp.zv);
    READ_VALUE(ifs, pp.etotps);

    ifs >> pp.ecutwfc >> pp.ecutrho;
    ifs.ignore(75, '\n');

    READ_VALUE(ifs, pp.lmax);
    READ_VALUE(ifs, pp.mesh);
    if (pp.mesh % 2 == 0)
    {
        pp.mesh -= 1;
...
void Pseudopot_upf::read_pseudo_nl(std::ifstream& ifs, Atom_pseudo& pp)
{
    ...
    else
    {
        ...
        for (int i = 0; i < pp.nbeta; i++)
        {
            ...
            ifs >> idum;
            READ_VALUE(ifs, pp.lll[i]);      // nl_1
            READ_VALUE(ifs, this->kbeta[i]); // nl_2
        ...
        if (pp.tvanp)
        {
            if (!SCAN_BEGIN(ifs, "<PP_QIJ>", false))
            {
                SCAN_BEGIN(ifs, "<PP_QIJ_WITH_L>", false);
            }
            // If nqf is not zero, Qij's inside rinner are computed using qfcoef's
            READ_VALUE(ifs, this->nqf);
            pp.nqlc = 2 * pp.lmax + 1;
            ...
            for (int nb = 0; nb < pp.nbeta; nb++)
            {
                ...
                for (int mb = nb; mb < pp.nbeta; mb++)
                {
                    ...
                    if (ldum != lm)
                    {
                        throw std::runtime_error("Pseudopot_upf::read_pseudo_nl, inconsistent angular momentum for Q_ij");
                        // ModuleBase::WARNING_QUIT("Pseudopot_upf::read_pseudo_nl",
                                                //  "inconsistent angular momentum for Q_ij");
                    }

                    READ_VALUE(ifs, pp.qqq(nb, mb));
